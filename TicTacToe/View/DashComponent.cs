    ...
    public void Display()
    {
        ...
        switch (_boardDash.Orientation)
        {
            case DashOrientation.Horizontal:
                for (int i = 0; i < _gameBoard.Size; i++) _gameBoard[i, _boardDash.Position].Piece = '-';
                break;
            case DashOrientation.Vertical:
                for (int i = 0; i < _gameBoard.Size; i++) _gameBoard[_boardDash.Position, i].Piece = '|';
                break;
            case DashOrientation.PrimaryDiagonal:
                for (int i = 0; i < _gameBoard.Size; i++) _gameBoard[i, i].Piece = '\\';
                break;
            case DashOrientation.SecondaryDiagonal:
                for (int i = 0; i < _gameBoard.Size; i++) _gameBoard[_gameBoard.Size - 1 - i, i].Piece = '/';
                break;
        }
    }
}